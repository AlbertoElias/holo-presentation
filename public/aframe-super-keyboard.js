...
// Taken from https://github.com/WandererOU/aframe-keyboard/blob/master/src/utils/events.js
...
AFRAME.registerComponent('super-keyboard', {
  ...
  init: function () {
    ...
    document.addEventListener('keydown', function (e) {
      if (e.defaultPrevented || e.repeat || !self.data.show) {
        return
      }
      ...
      if (e.key === 'Backspace') {
        ...
        self.el.setAttribute('super-keyboard', {value: value.slice(0, -1)});
      } else if (e.key === 'Enter') {
        self.accept();
      } else if (e.key.length > 1) {
        return
      } else {
        self.el.setAttribute('super-keyboard', {value: value + e.key});
      }
    });
